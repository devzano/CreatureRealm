...
import {
  fetchWeaponList,
  fetchWeaponDetail,
  type WeaponIndexItem as WeaponListItem,
  type WeaponDetail,
  type TreantNode,
  ...
  type MerchantRow,
  type KeyValueRow,
} from "@/lib/palworld/items/paldbWeapon";
...
export default function PalworldWeaponDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<WeaponListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const found =
          list.find((x: WeaponListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: WeaponListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchWeaponDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load weapon.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Weapon", [item?.name]);
  ...
  if (error) {
    return (
      <PageWrapper title="Weapon" scroll={false} headerLayout="inline">
