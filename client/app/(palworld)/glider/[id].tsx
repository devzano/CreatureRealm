...
import {
  fetchGliderList,
  fetchGliderDetail,
  type GliderIndexItem as GliderListItem,
  type GliderDetail,
  type TreantNode,
  ...
  type MerchantRow,
  type KeyValueRow,
} from "@/lib/palworld/items/paldbGlider";
...
function statFillIconName(key: string): string {
  ...
  if (k.includes("speed")) return "speedometer";
  if (k.includes("stamina")) return "run-fast";
  if (k.includes("gravity")) return "arrow-down-bold";
...
export default function PalworldGliderDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<GliderListItem | null>(null);
  const [detail, setDetail] = useState<GliderDetail | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchGliderList();

        const idTrim = String(id).trim();
        const found =
          list.find((x: GliderListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: GliderListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchGliderDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load glider.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Glider", [item?.name]);
  ...
  if (error) {
    return (
      <PageWrapper title="Glider" scroll={false} headerLayout="inline">
