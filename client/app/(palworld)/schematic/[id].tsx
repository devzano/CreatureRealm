...
import {
  ...
  type SchematicIndexItem as SchematicListItem,
...
/**
 * Fill-style icon mapping (MaterialCommunityIcons)
 * Important: "weight-kilogram" is not consistent across packs; use "scale-bathroom" instead.
 */
...
export default function PalworldSchematicDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<SchematicListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchSchematicList();

        const idTrim = String(id).trim();
        const found =
          list.find((x: SchematicListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: SchematicListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchSchematicDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load schematic.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Schematic", [item?.name]);
  ...
  if (error) {
    return (
      <PageWrapper title="Schematic" scroll={false} headerLayout="inline">
