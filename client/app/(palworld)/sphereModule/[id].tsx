// app/(palworld)/sphere-module/[id].tsx
...
import {
  fetchSphereModuleList,
  fetchSphereModuleDetail,
  type SphereModuleIndexItem as SphereModuleListItem,
...
export default function PalworldSphereModuleDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<SphereModuleListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchSphereModuleList();

        const idTrim = String(id).trim();
        const found =
          list.find((x: SphereModuleListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: SphereModuleListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchSphereModuleDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load sphere module.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Sphere Module", [item?.name]);
  ...
  if (error) {
    return (
      <PageWrapper title="Sphere Module" scroll={false} headerLayout="inline">
