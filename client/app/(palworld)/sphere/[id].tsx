 ...
 * NOTE: "weight" wasn't rendering because MCI doesn't reliably have `weight` in all versions.
 * Use `weight-kilogram` instead.
 */
...
export default function PalworldSphereDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<SphereListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchSphereList();
        const found =
          list.find((x) => String(x.slug).trim() === String(id).trim()) ??
          list.find((x) => String(x.slug).trim().endsWith(String(id).trim()));

        const d = await fetchSphereDetail(String(id));
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load sphere.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Sphere", [item?.name]);
  ...
  const cp = safeNum(item?.capturePower);
  ...
  const craftingMaterials = detail?.craftingMaterials ?? [];
  ...
  function KeyValueRows({ rows }: { rows: KeyValueRow[] }) {
    ...
    if (!has) return <Text className="text-white/45 text-[12px]">No entries.</Text>;
  ...
  function AboutSection({ description, stats }: { description: string | null; stats: KeyValueRow[] }) {
    const hasDesc = !!(description ?? "").trim();
    const hasStats = !!stats?.length;

    if (!hasDesc && !hasStats) return null;
      ...
      <View className="mt-4">
        <Section title="About">
          <View>
            ...
            {!!hasStats && (
              <View className={hasDesc ? "mt-4 pt-4 border-t border-white/5" : ""}>
      ...
      <View className="mt-4">
        <Section title="Produced At">
          {rows?.length ? (
            ...
            <Text className="text-white/45 text-[12px]">No entries.</Text>
          )}
        </Section>
      </View>
    );
  ...
  if (error) {
    return (
      <PageWrapper title="Sphere" scroll={false} headerLayout="inline">
  ...
  return (
    <PageWrapper scroll title={title} subtitle={subtitle} headerLayout="inline">
        ...
        <View className="p-4 flex-row items-center">
          ...
          <View className="ml-4 flex-1">
            ...
            <Text className="text-white/60 text-[12px] mt-1" numberOfLines={1}>
              Tech {tech ?? "—"} <Text className="text-white/40">•</Text> CP {cp ?? "—"}
            </Text>
      ...
      <AboutSection description={description} stats={stats} />
