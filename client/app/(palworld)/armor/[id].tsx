...
import {
  fetchArmorList,
  fetchArmorDetail,
  type ArmorIndexItem as ArmorListItem,
  type ArmorDetail,
  type TreantNode,
...
function isWorkIngredient(m: any) {
  ...
  // sometimes the icon is the only signal
...
export default function PalworldArmorDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<ArmorListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchArmorList();

        const idTrim = String(id).trim();
        const found =
          list.find((x: ArmorListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: ArmorListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchArmorDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load armor.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Armor", [item?.name]);
  ...
  if (error) {
    return (
      <PageWrapper title="Armor" scroll={false} headerLayout="inline">
