// app/(palworld)/consumable/[id].tsx
...
import {
  ...
  type ConsumableIndexItem as ConsumableListItem,
...
export default function PalworldConsumableDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string; }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<ConsumableListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchConsumableList();

        const idTrim = String(id).trim();
        const found =
          list.find((x: ConsumableListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: ConsumableListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchConsumableDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load consumable.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Consumable", [item?.name]);
  ...
  function FoodsSection({ rows }: { rows: KeyValueRow[]; }) {
    ...
    return (
      <View className="mt-6">
        <Section title="Foods" rightText="Food effects">
          <KeyValueRows rows={rows} />
        </Section>
      </View>
    );
  ...
  if (error) {
    return (
      <PageWrapper title="Consumable" scroll={false} headerLayout="inline">
