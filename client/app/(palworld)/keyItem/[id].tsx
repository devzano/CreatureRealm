...
import {
  fetchKeyItemList,
  fetchKeyItemDetail,
  type KeyItemIndexItem as KeyItemListItem,
  type KeyItemDetail,
  type TreantNode,
...
export default function PalworldKeyItemDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<KeyItemListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchKeyItemList();

        const idTrim = String(id).trim();
        const found =
          list.find((x: KeyItemListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: KeyItemListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchKeyItemDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load key item.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Key Item", [item?.name]);
...
function AboutSection({
  ...
  if (error) {
    return (
      <PageWrapper title="Key Item" scroll={false} headerLayout="inline">
