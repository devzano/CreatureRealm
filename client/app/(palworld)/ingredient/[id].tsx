// app/(palworld)/ingredient/[id].tsx
...
import {
  ...
  type IngredientIndexItem as IngredientListItem,
...
export default function PalworldIngredientDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<IngredientListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const found =
          list.find((x: IngredientListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: IngredientListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchIngredientDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load ingredient.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Ingredient", [item?.name]);
  ...
  if (error) {
    return (
      <PageWrapper title="Ingredient" scroll={false} headerLayout="inline">
