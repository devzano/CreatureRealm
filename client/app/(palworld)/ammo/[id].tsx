...
import {
  ...
  type MerchantRow,
  type KeyValueRow,
} from "@/lib/palworld/items/paldbAmmo";
...
export default function PalworldAmmoDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<AmmoListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchAmmoList();

        const idTrim = String(id).trim();
        const found =
          list.find((x: AmmoListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: AmmoListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchAmmoDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load ammo.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Ammo", [item?.name]);
  ...
  if (error) {
    return (
      <PageWrapper title="Ammo" scroll={false} headerLayout="inline">
