...
import {
  fetchMaterialList,
  fetchMaterialDetail,
  type MaterialIndexItem as MaterialListItem,
  type MaterialDetail,
  type TreantNode,
...
export default function PalworldMaterialDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();

  const [loading, setLoading] = useState(true);
  const [item, setItem] = useState<MaterialListItem | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const list = await fetchMaterialList();

        const idTrim = String(id).trim();
        const found =
          list.find((x: MaterialListItem) => String(x.slug).trim() === idTrim) ??
          list.find((x: MaterialListItem) => String(x.slug).trim().endsWith(idTrim));

        const d = await fetchMaterialDetail(idTrim);
        ...
        if (!mounted) return;
        setError(e?.message ? String(e.message) : "Failed to load material.");
        setItem(null);
        setDetail(null);
      } finally {
  ...
  const title = useMemo(() => item?.name ?? "Material", [item?.name]);
  ...
  if (error) {
    return (
      <PageWrapper title="Material" scroll={false} headerLayout="inline">
