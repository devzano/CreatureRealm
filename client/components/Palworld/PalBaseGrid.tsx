...
import type { BaseIndex, BaseLevelRow, BaseTask, BaseRewardKV, BaseItemRef } from "@/lib/palworld/paldbBase";
...
function renderTaskRow(t: BaseTask, idx: number) {
  ...
  if (t.kind === "build") {
    ...
    const text = safeStr((t as any).text);

    const icon = safeStr(item?.iconUrl);
    const name = safeStr(item?.name) || text.replace(/^Build\s+/i, "").replace(/\.\s*x\s*\d+\s*$/i, "").trim();
  ...
  const txt = safeStr((t as any).text);
  if (!txt) return null;
...
function BaseTile({
  ...
  const previewLines = tasks
    .map((t) => safeStr((t as any).text) || (t.kind === "pals" ? `Pals x${safeNum((t as any).count) ?? "—"}` : ""))
    .filter(Boolean)
    .slice(0, 2);
...
export default function PalBaseGrid({
  ...
  const rewards = rewardsRaw.filter((r) => !!(safeStr(r.key) || safeStr(r.valueText)));
    ...
    <>
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} bounces contentContainerStyle={{ paddingBottom: 26 }}>
          ...
          {/* Raw (optional safety net) */}
          {safeStr((sel as any)?.rawText) ? (
            <View className="mt-4">
              <SectionLabel>Notes</SectionLabel>
              <View className="rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-3">
                <Text className="text-white/65 text-[12px] leading-5">{safeStr((sel as any).rawText)}</Text>
              </View>
            </View>
          ) : null}
        </ScrollView>
