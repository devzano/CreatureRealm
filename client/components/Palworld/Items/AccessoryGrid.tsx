// components/palworld/AccessoryGrid.tsx
import React, { useMemo, useEffect, useState, useCallback, useRef } from "react";
import { View, Text, Pressable, ScrollView, ActivityIndicator } from "react-native";
import { Ionicons } from "@expo/vector-icons";

import {
  type AccessoryIndexItem,
...
type AccessoryGridProps = {
  items: AccessoryIndexItem[];
  onPressItem?: (item: AccessoryIndexItem) => void;
  emptyText?: string;
...
function stripTrailingAccessory(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*accessories?\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string) {
  const base = stripTrailingAccessory(nameRaw);
  const line1 = base || "Accessory";
  const line2 = "Accessory";
  return { line1, line2 };
}

function variantKey(it: Pick<AccessoryIndexItem, "slug" | "rarity">) {
...
export default function AccessoryGrid({
  ...
  const [selected, setSelected] = useState<AccessoryIndexItem | null>(null);

  const detailCache = useRef<Map<string, AccessoryDetail>>(new Map());
  ...
  const openSheet = useCallback(
    async (it: AccessoryIndexItem) => {
      ...
      try {
        const d = await fetchAccessoryDetail(slug);
  ...
  const production = useMemo(
    () => filterOutWorkFromRecipeRows((detail as any)?.production ?? []),
    [detail]
  );
