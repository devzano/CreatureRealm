// components/palworld/KeyItemGrid.tsx
import React, { useMemo, useEffect, useState, useCallback, useRef } from "react";
import { View, Text, Pressable, ScrollView, ActivityIndicator } from "react-native";
import { Ionicons, MaterialCommunityIcons } from "@expo/vector-icons";

import {
  type KeyItemIndexItem,
  type KeyItemDetail,
  type TreantNode,
  ...
  type KeyValueRow,
  fetchKeyItemDetail,
} from "@/lib/palworld/items/paldbKeyItems";

import RemoteIcon, { prefetchRemoteIcons } from "@/components/RemoteIcon";
import BottomSheetModal from "@/components/ui/BottomSheetModal";

type KeyItemGridProps = {
  items: KeyItemIndexItem[];
  onPressItem?: (item: KeyItemIndexItem) => void;
  emptyText?: string;
...
function stripTrailingKeyItem(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*key\s*items?\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string) {
  const base = stripTrailingKeyItem(nameRaw);
  const line1 = base || "Key Item";
  const line2 = "Key Item";
  return { line1, line2 };
}
...
function ProducedAtSection({ rows }: { rows: { slug: string; name: string; iconUrl: string | null; }[]; }) {
  ...
  return (
    <View className="mt-5">
      <SectionLabel>Produced At</SectionLabel>
...
function DroppedBySection({ rows }: { rows: DroppedByRow[]; }) {
  ...
  return (
    <View className="mt-5">
      <SectionLabel>Dropped By</SectionLabel>
      <View className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
        {rows.map((r, idx) => (
          <View
...
function TreasureBoxSection({ rows }: { rows: TreasureBoxRow[]; }) {
  ...
  return (
    <View className="mt-5">
      <SectionLabel>Treasure Box</SectionLabel>
      <View className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
        {rows.map((r, idx) => (
          <View
...
function WanderingMerchantSection({ rows }: { rows: MerchantRow[]; }) {
  ...
  return (
    <View className="mt-5">
      <SectionLabel>Wandering Merchant</SectionLabel>
      <View className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
        {rows.map((r, idx) => (
          <View
...
export default function KeyItemGrid({
  items,
  onPressItem,
  emptyText = "No key items found.",
  ...
  const [selected, setSelected] = useState<KeyItemIndexItem | null>(null);

  const detailCache = useRef<Map<string, KeyItemDetail>>(new Map());
  const [detail, setDetail] = useState<KeyItemDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: KeyItemIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchKeyItemDetail(key);
    ...
    <>
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} bounces contentContainerStyle={{ paddingBottom: 26 }}>
          ...
          {/* Dependency Tree (only if exists) */}
          {!!treant ? (
            <View className="mt-5">
              <SectionLabel>Dependency Tree</SectionLabel>
              <View className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
          ...
          {/* Produced At / Production / Crafting Materials */}
          ...
          {/* Dropped By / Treasure Box / Wandering Merchant */}
          ...
          {/* Quick Recipe (index fallback) — only if non-empty */}
                ...
                {selected.recipes.map((r, idx) => (
                  <View
                    ...
                    className={[
                      ...
                      idx !== selected.recipes.length - 1 ? "border-b border-white/5" : "",
                    ].join(" ")}
                  >
                    <View className="flex-row items-center flex-1">
                      <RemoteIcon
