// components/palworld/SphereGrid.tsx
...
// If name already includes "Sphere", strip it so we don't show "Sphere Sphere"
function stripTrailingSphere(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*sphere\s*$/i, "").trim();
}

// Build 2-line title: line1 = cleaned name (no Sphere), line2 = "Sphere"
function buildTwoLineTitle(nameRaw: string) {
  const base = stripTrailingSphere(nameRaw);
  const line1 = base || "Sphere";
  const line2 = "Sphere";
  return { line1, line2 };
}
 ...
 * NOTE: Use `weight-kilogram` for compatibility.
 */
function statFillIconName(key: string): string {
  ...
  if (k.includes("capture") || k === "cp") return "crosshairs-gps";
...
/** ---------- Sheet helpers (MATCH SCHEMATIC GRID) ---------- **/
...
export default function SphereGrid({
  ...
  const [selected, setSelected] = useState<SphereListItem | null>(null);

  const detailCache = useRef<Map<string, SphereDetail>>(new Map());
  const [detail, setDetail] = useState<SphereDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: SphereListItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchSphereDetail(key);
  ...
  const sheetCP = selected ? safeNum(selected.capturePower) : null;

  const TILE_H = 154;

  // Detail-driven (match schematic grid structure)
  const description = (detail as any)?.description ?? selected?.description ?? null;
    ...
    <>
      <View className="px-4">
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it) => {
            ...
            const cp = safeNum(it.capturePower);
                ...
                >
                  <View className="flex-1 px-3 pt-3 pb-3">
                    ...
                    <View className="flex-row items-end justify-between mt-2">
                      ...
                      <View className="flex-1 items-center">
                        <Text className="text-[10px] text-white/60">CP</Text>
                        <View className="mt-1 px-3 py-[3px] rounded-full border bg-white/5 border-white/10">
                          <Text className="text-[10px] text-white/85">{cp != null ? String(cp) : "—"}</Text>
                        </View>
                      </View>
                    </View>
                  </View>
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} bounces contentContainerStyle={{ paddingBottom: 26 }}>
          {/* Header (MATCH SCHEMATIC GRID: close button, not Details) */}
          <View className="flex-row items-center justify-between">
            <View className="flex-row items-center flex-1">
              <RemoteIcon
              ...
              <View className="ml-3 flex-1 pr-3">
                ...
                <Text className="text-white/60 text-[12px] mt-0.5" numberOfLines={1}>
                  Tech {sheetTech ?? "—"} <Text className="text-white/40">•</Text> CP {sheetCP ?? "—"}
                </Text>
              </View>
            </View>
          ...
          {/* Quick Recipe (optional, still styled like Schematic sections) */}
