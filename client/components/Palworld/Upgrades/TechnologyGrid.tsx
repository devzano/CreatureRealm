...
import { type TechnologyItem, warmTechnologies, getTechnologyPointTotals, fetchTechnologyHoverDetails } from "@/lib/palworld/upgrades/paldbTechnologies";
...
function ringClass(it: TechnologyItem) {
  ...
  const cat = safeText(it.category).toLowerCase();
...
export default function TechnologyGrid({
  ...
  useEffect(() => {
    ...
    const slug = safeText(selected?.slug);
  ...
  const selCategory = safeText(sel?.category) || "Unknown";
  const selLevelList = safeNum(sel?.level);
  const selSlug = safeText(sel?.slug) || null;
  ...
  const hoverDesc = safeText(hoverDetails?.description);
    ...
    <>
      <View className="px-4">
        <View className="flex-row flex-wrap -mx-2">
          ...
          {list.map((it) => {
            const title = safeText(it.name) || "Technology";
            const subtitle = safeText(it.category) || "—";
