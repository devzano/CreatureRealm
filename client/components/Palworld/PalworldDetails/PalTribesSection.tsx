...
export const PalTribesSection: React.FC<PalTribesSectionProps> = ({
  tribes,
}) => {
  const rows = tribes ?? [];
      ...
      {rows.map((t, idx) => (
        ...
        >
          <View className="flex-row items-center flex-1 pr-3">
            ...
            <Text className="text-[12px] text-slate-100 font-semibold flex-1">
              {t.palName}
            </Text>
          </View>

          <Text className="text-[12px] text-slate-400 font-semibold text-right">
            {t.tribeRole}
          </Text>
        </View>
      ))}
