// components/PalworldMap/PalworldMapModal.tsx
...
type PalworldMapModalProps = {
  visible: boolean;
  onClose: () => void;
  title: string; // e.g. "Palpagos Islands Map"
  subtitle: string; // e.g. "Palworld"
  url: string; // map URL
  disclaimer?: string;
  mapCategoryToggles?: MapCategoryToggle[];
};

const PalworldMapModal: React.FC<PalworldMapModalProps> = ({
  visible,
  onClose,
  title,
  subtitle,
        ...
        setCategoryToggleState((prev) => {
          ...
          for (const toggle of mapCategoryToggles) {
            ...
            if (toggle.kind === "parent") {
              active = values.length > 0 && values.every((v) => parentSet.has(v));
            } else if (toggle.kind === "sub") {
              active = values.length > 0 && values.every((v) => subSet.has(v));
            } else if (toggle.kind === "item") {
              active = values.length > 0 && values.every((v) => itemSet.has(v));
            }
  ...
  const toggleCategory = (toggle: MapCategoryToggle) => {
    setCategoryToggleState((prev) => {
      ...
      runScript(
        buildCRMapScript(
          `window.CRMapControls.${method}(${valuesArg}, ${
  ...
  const renderCategoryIcon = (id: string, active: boolean) => {
    ...
    // keep the Pokémon heuristics, add Palworld-ish ones
    if (lower.includes("item")) name = "backpack";
    else if (lower.includes("resource") || lower.includes("ore") || lower.includes("wood"))
      name = "forest";
    else if (lower.includes("boss") || lower.includes("tower"))
      name = "sports-martial-arts";
    else if (lower.includes("dungeon") || lower.includes("cave"))
      name = "terrain";
    else if (lower.includes("egg")) name = "egg";
    else if (lower.includes("chest") || lower.includes("loot"))
      name = "inventory-2";
    else if (lower.includes("pal")) name = "pets";
    else if (lower.includes("poi") || lower.includes("location"))
      name = "place";
    ...
    <View style={styles.overlay} pointerEvents="box-none">
      ...
      >
        ...
        <View style={styles.controlsWrapper}>
          {/* Primary row: Hide all, Labels, + category chips */}
          <ScrollView
            horizontal
              ...
              <WebView
                ...
                onLoadEnd={() => {
                  ...
                  if (webViewRef.current) {
                    ...
                    webViewRef.current.injectJavaScript(MAP_CONTROL_HELPERS_JS);
                  }
                }}
                onError={() => {
                  setHasError(true);
...
const styles = StyleSheet.create({
  ...
  metaStrip: {
    backgroundColor: "rgba(234,88,12,0.16)", // orange-ish for Palworld
    marginHorizontal: 16,
    marginBottom: 8,
    borderRadius: 999,
    paddingHorizontal: 10,
  ...
  chipActive: {
    borderColor: "rgba(249,115,22,0.9)", // orange active
    backgroundColor: "rgba(67,20,7,0.95)",
  },
  chipText: {
    fontSize: 12,
    color: "#E5E7EB",
...
export default PalworldMapModal;
