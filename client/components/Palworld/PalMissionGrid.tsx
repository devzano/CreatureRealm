...
import type {
  ...
  MissionCoord,
  MissionKind,
} from "@/lib/palworld/paldbMissions";
...
type PalMissionGridProps = {
  ...
  defaultTab?: MissionKind; // still accepted but no longer used
};
...
function tileKey(it: MissionIndexItem) {
  return safeStr(it.id) || safeStr(it.title) || Math.random().toString(36).slice(2);
}
...
function renderEntityRow(e: MissionEntityRef, idx: number) {
  ...
  const name = safeStr((e as any).name);
  const icon = safeStr((e as any).icon);
...
function MissionTile({
  ...
  const title = safeStr(it.title) || "Mission";
...
export default function PalMissionGrid({
  ...
  // show whole list: main then sub (preserve each list’s order)
  const list = useMemo(() => {
  ...
  const missionKey = sel ? `mission:${safeStr(sel.id)}` : "";
  const collected = missionKey ? (storeSnap as any).getEntry(missionKey)?.mission : false;

  const descText = safeStr(sel?.description);

  const objectivesRaw = Array.isArray(sel?.objectives) ? (sel!.objectives as MissionEntityRef[]) : [];
  const objectives = objectivesRaw.filter((e) => (e.type === "text" ? !!safeStr((e as any).text) : true));
  const hasObjectives = objectives.some((e) =>
    e.type === "text" ? !!safeStr((e as any).text) : !!safeStr((e as any).name)
  );
  ...
  const nextTitle = safeStr(sel?.nextTitle);
