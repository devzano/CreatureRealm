...
const stripTrailingStorage = makeStripTrailingSuffix(/\s*storage\s*$/i);
...
export default function PalStorageGrid({
  ...
  const { visible: sheetVisible, open: openSheet, close: closeSheet, selected, detail, loading, error } =
    useDetailSheet<StorageIndexItem, StorageDetail>(fetchStorageDetail, onPressItem);
            ...
            <View className="flex-row items-center flex-1">
              ...
              <View className="ml-3 flex-1 pr-3">
                ...
                <Text className="text-white/60 text-[12px] mt-0.5" numberOfLines={1}>
                  Tech {tech != null ? formatPillNumber(tech) : "—"} <Text className="text-white/40">•</Text> Slots{" "}
                  {slots != null ? formatPillNumber(slots) : "—"}
                  {categoryText ? (
                    <>
                      {" "}
