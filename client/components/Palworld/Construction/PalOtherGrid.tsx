// components/Palworld/PalOtherGrid.tsx
...
import { fetchOtherDetail, type OtherIndexItem, type OtherDetail } from "@/lib/palworld/construction/paldbOther";
import type { TreantNode } from "@/lib/palworld/paldbDetailKit";

type PalOtherGridProps = {
  items: OtherIndexItem[];
  onPressItem?: (item: OtherIndexItem) => void;
...
function stripTrailingOther(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*other\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingOther(nameRaw);
  const line1 = base || "Other";
  const line2 = safeText(categoryText) || "Other";
  return { line1, line2 };
}
        ...
        {list.map((r, idx) => {
          ...
          return (
            <View
              key={key}
              className={[
                "flex-row items-center justify-content-between py-2 px-3",
...
export default function PalOtherGrid({
  items,
  onPressItem,
  emptyText = "No other builds found.",
  ...
  const [selected, setSelected] = useState<OtherIndexItem | null>(null);

  const detailCache = useRef<Map<string, OtherDetail>>(new Map());
  const [detail, setDetail] = useState<OtherDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: OtherIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchOtherDetail(key);
              ...
              <View key={String(it?.slug)} className="px-2 mb-4" style={{ width: `${100 / cols}%` as any }}>
                ...
                >
                  <View className="flex-1 px-3 pt-3 pb-3">
                    ...
                    {/* Footer pills (Tech only — removed Slots) */}
                    <View className="mt-2 items-center">
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Text className="text-white/80 text-[12px]">
                Type: <Text className="text-white">Other</Text>
              </Text>
            </View>
