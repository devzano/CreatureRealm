...
const stripTrailingInfrastructure = makeStripTrailingSuffix(/\s*infrastructure\s*$/i);
...
export default function PalInfrastructureGrid({
  ...
  const { visible: sheetVisible, open: openSheet, close: closeSheet, selected, detail, loading, error } =
    useDetailSheet<InfrastructureIndexItem, InfrastructureDetail>(fetchInfrastructureDetail, onPressItem);
        ...
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const title = buildTwoLineTitle(safeName(it), it?.categoryText ?? null, {
              strip: stripTrailingInfrastructure,
              fallback1: "Infrastructure",
              fallback2: "Infrastructure",
            });
