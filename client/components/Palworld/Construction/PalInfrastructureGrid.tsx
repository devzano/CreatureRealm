// components/Palworld/PalInfrastructureGrid.tsx
...
import {
  ...
  type InfrastructureIndexItem,
  type InfrastructureDetail,
} from "@/lib/palworld/construction/paldbInfrastructure";
import type { TreantNode } from "@/lib/palworld/paldbDetailKit";

type PalInfrastructureGridProps = {
  items: InfrastructureIndexItem[];
  onPressItem?: (item: InfrastructureIndexItem) => void;
...
function stripTrailingInfrastructure(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*infrastructure\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingInfrastructure(nameRaw);
  const line1 = base || "Infrastructure";
  const line2 = safeText(categoryText) || "Infrastructure";
  return { line1, line2 };
}
...
function formatSigned(v: any): string {
  const n = safeNum(v);
  if (n == null) return "—";
...
export default function PalInfrastructureGrid({
  items,
  onPressItem,
  emptyText = "No infrastructure found.",
  showUnavailable = false,
  numColumns = 3,
  prefetchIcons = true,
}: PalInfrastructureGridProps) {
  ...
  const [selected, setSelected] = useState<InfrastructureIndexItem | null>(null);

  const detailCache = useRef<Map<string, InfrastructureDetail>>(new Map());
  const [detail, setDetail] = useState<InfrastructureDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: InfrastructureIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchInfrastructureDetail(key);
  ...
  const healing = safeNum((d as any)?.healing ?? (sel as any)?.healing);
  const san = safeNum((d as any)?.san ?? (sel as any)?.san);
  ...
  // Tile secondary stat: prefer SAN, else Healing, else —
  const secondary = san != null ? san : healing != null ? healing : null;
  const secondaryLabel = san != null ? "SAN" : healing != null ? "Heal" : "Stat";
      ...
      <View className="px-4">
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const h = safeNum(it?.healing);
            const sn = safeNum(it?.san);

            const sec = sn != null ? sn : h != null ? h : null;
            const secLabel = sn != null ? "SAN" : h != null ? "Heal" : "Stat";
              ...
              <View className="ml-3 flex-1 pr-3">
                ...
                <Text className="text-white/60 text-[12px] mt-0.5" numberOfLines={1}>
                  ...
                  {san != null ? (
                    <>
                      {" "}
                      <Text className="text-white/40">•</Text> SAN {formatSigned(san)}
                    </>
                  ) : null}
                  {healing != null ? (
                    <>
                      {" "}
                      <Text className="text-white/40">•</Text> Healing {formatSigned(healing)}
                    </>
                  ) : null}
                  {categoryText ? (
                    <>
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Text className="text-white/80 text-[12px]">
                Type: <Text className="text-white">Infrastructure</Text>
              </Text>
            </View>
            ...
            {san != null ? (
              <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
                <Text className="text-white/80 text-[12px]">
                  SAN: <Text className="text-white">{formatSigned(san)}</Text>
                </Text>
              </View>
            ) : null}
            ...
            {healing != null ? (
              <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
                <Text className="text-white/80 text-[12px]">
                  Healing: <Text className="text-white">{formatSigned(healing)}</Text>
                </Text>
              </View>
            ) : null}
