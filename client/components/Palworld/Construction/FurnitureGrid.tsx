...
import {
  ...
  type FurnitureIndexItem,
  type FurnitureDetail,
} from "@/lib/palworld/construction/paldbFurniture";
...
const stripTrailingFurniture = makeStripTrailingSuffix(/\s*furniture\s*$/i);
...
export default function PalFurnitureGrid({
  ...
  const { visible: sheetVisible, open: openSheet, close: closeSheet, selected, detail, loading, error } =
    useDetailSheet<FurnitureIndexItem, FurnitureDetail>(fetchFurnitureDetail, onPressItem);
        ...
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const title = buildTwoLineTitle(safeName(it), it?.categoryText ?? null, {
              strip: stripTrailingFurniture,
              fallback1: "Furniture",
              fallback2: "Furniture",
            });

            const t = safeNum(it?.technologyLevel ?? it?.technology);
            const mats = sumRecipeQty((it?.recipe ?? []) as any[]); // kept (currently unused)
