// components/Palworld/PalFoodGrid.tsx
...
import { fetchFoodDetail, type FoodIndexItem, type FoodDetail } from "@/lib/palworld/construction/paldbFood";
import type { TreantNode } from "@/lib/palworld/paldbDetailKit";

type PalFoodGridProps = {
  items: FoodIndexItem[];
  onPressItem?: (item: FoodIndexItem) => void;
...
function stripTrailingFood(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*food\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingFood(nameRaw);
  const line1 = base || "Food";
  const line2 = safeText(categoryText) || "Food";
  return { line1, line2 };
}
...
function formatSan(v: any): string {
  const n = safeNum(v);
  if (n == null) return "—";
  // show sign with up to 2 decimals (matches PalDB feel)
...
function WorkSuitabilitySection({
  ws,
}: {
  ws:
    | null
      ...
      <View className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
        <View className="flex-row items-center justify-between py-3 px-3">
          <View className="flex-row items-center flex-1 pr-3">
            <RemoteIcon
              uri={(ws as any)?.iconUrl ?? null}
              size={22}
              roundedClassName="rounded-md"
              placeholderClassName="bg-white/5 border border-white/10"
              contentFit="contain"
            />
            <Text className="ml-2 text-white/85 text-[12px] font-semibold" numberOfLines={1}>
              {name || "—"}
            </Text>
          </View>
...
export default function PalFoodGrid({
  items,
  onPressItem,
  ...
  const [selected, setSelected] = useState<FoodIndexItem | null>(null);

  const detailCache = useRef<Map<string, FoodDetail>>(new Map());
  const [detail, setDetail] = useState<FoodDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: FoodIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchFoodDetail(key);
  ...
  const workSuitability = (d as any)?.workSuitability ?? (sel as any)?.workSuitability ?? null;
  const wsLevel = safeNum((workSuitability as any)?.level);
    ...
    <>
      <View className="px-4">
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const wsLvl = safeNum(it?.workSuitability?.level);
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} bounces contentContainerStyle={{ paddingBottom: 26 }}>
              ...
              <View className="ml-3 flex-1 pr-3">
                ...
                <Text className="text-white/60 text-[12px] mt-0.5" numberOfLines={1}>
                  ...
                  {wsLevel != null ? (
                    <>
                      {" "}
                      <Text className="text-white/40">•</Text> Work Lv {formatPillNumber(wsLevel)}
                    </>
                  ) : null}
                  {san != null ? (
                    <>
                      {" "}
                      <Text className="text-white/40">•</Text> SAN {formatSan(san)}
                    </>
                  ) : null}
                  {categoryText ? (
                    <>
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Text className="text-white/80 text-[12px]">
                Type: <Text className="text-white">Food</Text>
              </Text>
            </View>
            ...
            {workSuitability?.name ? (
              <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
                <Text className="text-white/80 text-[12px]">
                  Work:{" "}
                  <Text className="text-white">
                    {workSuitability.name}
                    {wsLevel != null ? ` Lv ${formatPillNumber(wsLevel)}` : ""}
                  </Text>
                </Text>
              </View>
            ) : null}
            ...
            {san != null ? (
              <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
                <Text className="text-white/80 text-[12px]">
                  SAN: <Text className="text-white">{formatSan(san)}</Text>
                </Text>
              </View>
            ) : null}
          ...
          <WorkSuitabilitySection ws={workSuitability} />
