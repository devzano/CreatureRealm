// components/Palworld/PalFurnitureGrid.tsx
...
import { fetchFurnitureDetail, type FurnitureIndexItem, type FurnitureDetail } from "@/lib/palworld/construction/paldbFurniture";
import type { TreantNode } from "@/lib/palworld/paldbDetailKit";

type PalFurnitureGridProps = {
  items: FurnitureIndexItem[];
  onPressItem?: (item: FurnitureIndexItem) => void;
...
function stripTrailingFurniture(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*furniture\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingFurniture(nameRaw);
  const line1 = base || "Furniture";
  const line2 = safeText(categoryText) || "Furniture";
  return { line1, line2 };
}
...
export default function PalFurnitureGrid({
  items,
  onPressItem,
  emptyText = "No furniture found.",
  showUnavailable = false,
  numColumns = 3,
  prefetchIcons = true,
}: PalFurnitureGridProps) {
  ...
  const [selected, setSelected] = useState<FurnitureIndexItem | null>(null);

  const detailCache = useRef<Map<string, FurnitureDetail>>(new Map());
  const [detail, setDetail] = useState<FurnitureDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: FurnitureIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchFurnitureDetail(key);
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Text className="text-white/80 text-[12px]">
                Type: <Text className="text-white">Furniture</Text>
              </Text>
            </View>
