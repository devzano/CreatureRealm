// components/Palworld/PalStorageGrid.tsx
...
import { fetchStorageDetail, type StorageIndexItem, type StorageDetail } from "@/lib/palworld/construction/paldbStorage";
import type { TreantNode } from "@/lib/palworld/paldbDetailKit";

type PalStorageGridProps = {
  items: StorageIndexItem[];
  onPressItem?: (item: StorageIndexItem) => void;
...
function stripTrailingStorage(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*storage\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingStorage(nameRaw);
  const line1 = base || "Storage";
  const line2 = safeText(categoryText) || "Storage";
  return { line1, line2 };
}
...
export default function PalStorageGrid({
  items,
  onPressItem,
  ...
  const [selected, setSelected] = useState<StorageIndexItem | null>(null);

const detailCache = useRef<Map<string, StorageDetail>>(new Map());
const [detail, setDetail] = useState<StorageDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: StorageIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchStorageDetail(key);
  ...
  const slots = safeNum((d as any)?.slots ?? (sel as any)?.slots);
    ...
    <>
      <View className="px-4">
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const s = safeNum(it?.slots);

            // storage has no rarity ring, keep default like "common"
                ...
                >
                  <View className="flex-1 px-3 pt-3 pb-3">
                    ...
                    <View className="flex-row items-end justify-between mt-2">
                      ...
                      <View className="flex-1 items-center">
                        <Text className="text-[10px] text-white/60">Slots</Text>
                        <View className="mt-1 px-3 py-[3px] rounded-full border bg-white/5 border-white/10">
                          <Text
                            className="text-[10px] text-white/85"
                            numberOfLines={1}
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} bounces contentContainerStyle={{ paddingBottom: 26 }}>
          {/* Header (close button like GliderGrid) */}
            ...
            <View className="flex-row items-center flex-1">
              ...
              <View className="ml-3 flex-1 pr-3">
                ...
                <Text className="text-white/60 text-[12px] mt-0.5" numberOfLines={1}>
                  Tech {tech != null ? formatPillNumber(tech) : "—"} <Text className="text-white/40">•</Text>{" "}
                  Slots {slots != null ? formatPillNumber(slots) : "—"}
                  {categoryText ? (
                    <>
                      {" "}
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Text className="text-white/80 text-[12px]">
                Type: <Text className="text-white">Storage</Text>
              </Text>
            </View>
            ...
            {slots != null ? (
              <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
                <Text className="text-white/80 text-[12px]">
                  Slots: <Text className="text-white">{formatPillNumber(slots)}</Text>
                </Text>
              </View>
            ) : null}
