...
import {
  ...
  type ProductionIndexItem,
  type ProductionDetail,
} from "@/lib/palworld/construction/paldbProduction";
...
const stripTrailingProduction = makeStripTrailingSuffix(/\s*production\s*$/i);
...
export default function PalProductionGrid({
  ...
  const { visible: sheetVisible, open: openSheet, close: closeSheet, selected, detail, loading, error } =
    useDetailSheet<ProductionIndexItem, ProductionDetail>(fetchProductionDetail, onPressItem);
  ...
  const wsLevel = safeNum(workSuitability?.level);
        ...
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const title = buildTwoLineTitle(safeName(it), it?.categoryText ?? null, {
              strip: stripTrailingProduction,
              fallback1: "Production",
              fallback2: "Production",
            });
