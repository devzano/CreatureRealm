// components/Palworld/PalProductionGrid.tsx
...
import { fetchProductionDetail, type ProductionIndexItem, type ProductionDetail } from "@/lib/palworld/construction/paldbProduction";
import type { TreantNode } from "@/lib/palworld/paldbDetailKit";

type PalProductionGridProps = {
  items: ProductionIndexItem[];
  onPressItem?: (item: ProductionIndexItem) => void;
...
function stripTrailingProduction(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*production\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingProduction(nameRaw);
  const line1 = base || "Production";
  const line2 = safeText(categoryText) || "Production";
  return { line1, line2 };
}
...
export default function PalProductionGrid({
  items,
  onPressItem,
  emptyText = "No production found.",
  showUnavailable = false,
  numColumns = 3,
  prefetchIcons = true,
}: PalProductionGridProps) {
  ...
  const [selected, setSelected] = useState<ProductionIndexItem | null>(null);

  const detailCache = useRef<Map<string, ProductionDetail>>(new Map());
  const [detail, setDetail] = useState<ProductionDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: ProductionIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchProductionDetail(key);
  ...
  const ws = (d as any)?.workSuitability ?? (sel as any)?.workSuitability ?? null;
  const wsLevel = safeNum(ws?.level);
      ...
      <View className="px-4">
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const wsl = safeNum(it?.workSuitability?.level);
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Text className="text-white/80 text-[12px]">
                Type: <Text className="text-white">Production</Text>
              </Text>
            </View>
            ...
            {wsLevel != null ? (
              <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
                <Text className="text-white/80 text-[12px]">
                  Work: <Text className="text-white">Lv {formatPillNumber(wsLevel)}</Text>
                </Text>
              </View>
            ) : null}
