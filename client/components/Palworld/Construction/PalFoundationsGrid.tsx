// components/Palworld/PalFoundationsGrid.tsx
...
import {
  fetchFoundationsDetail,
  type FoundationsIndexItem,
  type FoundationsDetail,
} from "@/lib/palworld/construction/paldbFoundations";
import type { TreantNode } from "@/lib/palworld/paldbDetailKit";

type PalFoundationsGridProps = {
  items: FoundationsIndexItem[];
  onPressItem?: (item: FoundationsIndexItem) => void;
...
function stripTrailingFoundations(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;

  // keep it light: remove "Foundation(s)" suffix if present
  return s.replace(/\s*foundations?\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingFoundations(nameRaw);
  const line1 = base || "Foundation";
  const line2 = safeText(categoryText) || "Foundations";
  return { line1, line2 };
}
...
export default function PalFoundationsGrid({
  items,
  onPressItem,
  emptyText = "No foundations found.",
  showUnavailable = false,
  numColumns = 3,
  prefetchIcons = true,
}: PalFoundationsGridProps) {
  ...
  const [selected, setSelected] = useState<FoundationsIndexItem | null>(null);

  const detailCache = useRef<Map<string, FoundationsDetail>>(new Map());
  const [detail, setDetail] = useState<FoundationsDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: FoundationsIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchFoundationsDetail(key);
      ...
      <View className="px-4">
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            // Foundations: no rarity ring; keep neutral
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Text className="text-white/80 text-[12px]">
                Type: <Text className="text-white">Foundations</Text>
              </Text>
            </View>
