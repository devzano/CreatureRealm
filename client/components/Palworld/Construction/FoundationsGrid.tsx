...
const stripTrailingFoundations = makeStripTrailingSuffix(/\s*foundations?\s*$/i);
...
export default function PalFoundationsGrid({
  ...
  const { visible: sheetVisible, open: openSheet, close: closeSheet, selected, detail, loading, error } =
    useDetailSheet<FoundationsIndexItem, FoundationsDetail>(fetchFoundationsDetail, onPressItem);
        ...
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const title = buildTwoLineTitle(safeName(it), it?.categoryText ?? null, {
              strip: stripTrailingFoundations,
              fallback1: "Foundation",
              fallback2: "Foundations",
            });
