// components/Palworld/PalPalGrid.tsx
...
function stripTrailingPal(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*pal\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingPal(nameRaw);
  const line1 = base || "Pal";
  const line2 = safeText(categoryText) || "Pal";
  return { line1, line2 };
}
...
export default function PalPalGrid({
  ...
  const [selected, setSelected] = useState<PalConstructionIndexItem | null>(null);

  const detailCache = useRef<Map<string, PalDetail>>(new Map());
  const [detail, setDetail] = useState<PalDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: PalConstructionIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchPalDetail(key);
