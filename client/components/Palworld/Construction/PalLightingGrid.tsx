// components/Palworld/PalLightingGrid.tsx
...
import { fetchLightingDetail, type LightingIndexItem, type LightingDetail } from "@/lib/palworld/construction/paldbLighting";
import type { TreantNode } from "@/lib/palworld/paldbDetailKit";

type PalLightingGridProps = {
  items: LightingIndexItem[];
  onPressItem?: (item: LightingIndexItem) => void;
...
function stripTrailingLighting(name: string) {
  const s = String(name ?? "").replace(/\s+/g, " ").trim();
  if (!s) return s;
  return s.replace(/\s*lighting\s*$/i, "").trim();
}

function buildTwoLineTitle(nameRaw: string, categoryText?: string | null) {
  const base = stripTrailingLighting(nameRaw);
  const line1 = base || "Lighting";
  const line2 = safeText(categoryText) || "Lighting";
  return { line1, line2 };
}
...
export default function PalLightingGrid({
  items,
  onPressItem,
  ...
  const [selected, setSelected] = useState<LightingIndexItem | null>(null);

  const detailCache = useRef<Map<string, LightingDetail>>(new Map());
  const [detail, setDetail] = useState<LightingDetail | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  const [detailError, setDetailError] = useState<string | null>(null);

  const openSheet = useCallback(
    async (it: LightingIndexItem) => {
      onPressItem?.(it);
      ...
      try {
        const d = await fetchLightingDetail(key);
  ...
  const consumption = safeNum((d as any)?.consumption ?? (sel as any)?.consumption);
      ...
      <View className="px-4">
        <View className="flex-row flex-wrap -mx-2">
          {filtered.map((it: any) => {
            ...
            const c = safeNum(it?.consumption);
              ...
              <View className="ml-3 flex-1 pr-3">
                ...
                <Text className="text-white/60 text-[12px] mt-0.5" numberOfLines={1}>
                  ...
                  {consumption != null ? (
                    <>
                      {" "}
                      <Text className="text-white/40">•</Text> Cons. {formatSigned(consumption)}
                    </>
                  ) : null}
                  {categoryText ? (
                    <>
          ...
          <View className="mt-4 flex-row flex-wrap gap-2">
            <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Text className="text-white/80 text-[12px]">
                Type: <Text className="text-white">Lighting</Text>
              </Text>
            </View>
            ...
            {consumption != null ? (
              <View className="px-3 py-2 rounded-full border border-white/10 bg-white/5">
                <Text className="text-white/80 text-[12px]">
                  Consumption: <Text className="text-white">{formatSigned(consumption)}</Text>
                </Text>
              </View>
            ) : null}
