// components/AnimalCrossing/ACPhotosGrid.tsx
import React, { useCallback, useEffect, useMemo, useState } from "react";
import { View, Text, ActivityIndicator, Image, Pressable } from "react-native";
import { Feather } from "@expo/vector-icons";
import { useRouter } from "expo-router";

import { fetchPhotoByName, fetchPhotoNames, type NookipediaPhotoItem } from "@/lib/animalCrossing/nookipediaPhotos";
...
type PhotoTileProps = {
  name: string;
  detail?: NookipediaPhotoItem;
  usedThumb?: number;
  isDetailLoading: boolean;
  onNeedFallbackThumb: (name: string) => Promise<void>;
};

const PhotoTile: React.FC<PhotoTileProps> = React.memo(
  ({ name, detail, usedThumb, isDetailLoading, onNeedFallbackThumb }) => {
    const router = useRouter();

    const key = useMemo(() => acMakeKey("photo", name), [name]);
    ...
    const setCount = useAnimalCrossingCollectionStore(
      useCallback((s: any) => s.setCount, [])
    );

    const isCollected = !!entry?.collected;
    const count = Math.max(Number(entry?.count || 0), 0);

    const candidates = useMemo(() => buildPhotoImageCandidates(detail), [detail]);
    ...
    const category = detail?.category ? String((detail as any).category) : "Photo";
      ...
      <View className="w-1/3 p-1">
        <View className="rounded-3xl p-3 border mb-1 bg-slate-900/80 border-slate-700 items-center">
          ...
          <View className="flex-row items-center mt-2">
            <Pressable
              onPress={() => {
                toggleCollected("photo", name);
                // If turning on and you use counts, make first collect imply 1
                if (!isCollected && count <= 0) setCount("photo", name, 1);
              }}
            ...
            {isCollected ? (
              <View className="flex-row items-center ml-2">
                <Pressable
                  onPress={() => decrementCount("photo", name)}
                ...
                <Pressable
                  onPress={() => incrementCount("photo", name)}
                  className="w-6 h-6 rounded-xl bg-slate-950/60 border border-slate-700 items-center justify-center"
                >
                  <Text className="text-slate-100 text-[12px] font-bold">+</Text>
                </Pressable>
...
const ACPhotosGrid: React.FC<ACPhotosGridProps> = ({ search, collectedOnly = false, collectedIds }) => {
  ...
  const grid = useACNameDetailGrid<NookipediaPhotoItem>({
    search,
    fetchNames: fetchPhotoNames,
    fetchDetail: fetchPhotoByName,
    pageSize: PAGE_SIZE,
  ...
  return (
    <ACGridWrapper<string>
      ...
      initialLoadingText="Loading photo list…"
      errorText={grid.namesError}
      onRetry={grid.retryReloadNames}
      isEmpty={grid.filteredNames.length === 0}
      emptyText={collectedOnly ? "No collected photos in this category." : "No photos match this search."}
