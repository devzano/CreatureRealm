...
const ACPhotosGrid: React.FC<ACPhotosGridProps> = ({ search, collectedOnly = false, collectedIds }) => {
  ...
  const collectedSet = useMemo(() => {
    ...
    for (const [key, entry] of Object.entries(entries || {})) {
      ...
      const name = String(key).slice("photo:".length).trim();
  ...
  const headerLine = useMemo(() => {
    const label = collectedOnly ? "collected photos" : "photos";
  ...
  return (
    <ACGridWrapper<string>
      ...
      initialLoadingText={collectedOnly ? "Loading your collected photos…" : "Loading photo list…"}
