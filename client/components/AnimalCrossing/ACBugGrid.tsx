...
const BugTile: React.FC<BugTileProps> = React.memo(
...
const ACBugGrid: React.FC<ACBugGridProps> = ({ search, collectedOnly = false }) => {
  ...
  const collectedNameSet = useMemo(() => {
    ...
    for (const [key, entry] of Object.entries(src)) {
      ...
      const name = String(key).slice("bug:".length);
  ...
  const headerLine = useMemo(() => {
    ...
    return collectedOnly
      ? `Collected: ${grid.filteredNames.length} bugs${loading}`
      : `Showing ${grid.visibleNames.length} / ${grid.filteredNames.length} bugs${loading}`;
  ...
  return (
    <ACGridWrapper<string>
      ...
      initialLoadingText="Loading bug list…"
