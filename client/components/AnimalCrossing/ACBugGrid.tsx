// components/AnimalCrossing/ACBugGrid.tsx
...
import { fetchBugByName, fetchBugNames, type NookipediaBugItem } from "@/lib/animalCrossing/nookipediaBugs";
...
type BugTileProps = {
  name: string;
  detail?: NookipediaBugItem;
  usedThumb?: number;
  isDetailLoading: boolean;
  onNeedFallbackThumb: (name: string) => Promise<void>;
};

const BugTile: React.FC<BugTileProps> = React.memo(({ name, detail, usedThumb, isDetailLoading, onNeedFallbackThumb }) => {
  const router = useRouter();

  const key = useMemo(() => acMakeKey("bug", name), [name]);
  ...
  const candidates = useMemo(() => buildBugImageCandidates(detail), [detail]);
  ...
  const subtitle = detail?.location ? String((detail as any).location) : "Bug";
  ...
  return (
    <View className="w-1/3 p-1">
      <View className="rounded-3xl p-3 border mb-1 bg-slate-900/80 border-slate-700 items-center">
        ...
        <View className="mt-2">
          <Pressable
            onPress={() => toggleCollected("bug", name)}
...
const ACBugGrid: React.FC<ACBugGridProps> = ({ search, collectedOnly = false, collectedIds }) => {
  ...
  const collectedSet = useMemo(() => {
    ...
    for (const [key, entry] of Object.entries(entries || {})) {
      if (!String(key).startsWith("bug:")) continue;
      ...
      const name = String(key).slice("bug:".length).trim();
  ...
  const grid = useACNameDetailGrid<NookipediaBugItem>({
    search,
    fetchNames: fetchBugNames,
    fetchDetail: fetchBugByName,
    pageSize: PAGE_SIZE,
  ...
  const headerLine = useMemo(() => {
    ...
    const label = collectedOnly ? "collected" : "bugs";
  ...
  const renderBugItem = useCallback(
  ...
  return (
    <ACGridWrapper<string>
      ...
      initialLoadingText={collectedOnly ? "Loading your collected bugs…" : "Loading bug list…"}
      errorText={grid.namesError}
      onRetry={grid.retryReloadNames}
      isEmpty={grid.filteredNames.length === 0}
      emptyText={collectedOnly ? "No collected bugs yet." : "No bugs match this search."}
      headerLine={headerLine}
      data={grid.visibleNames}
      keyExtractor={(item) => item}
      renderItem={renderBugItem as any}
