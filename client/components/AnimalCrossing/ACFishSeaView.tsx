...
export default function ACFishSeaView({ search, initialMode = "fish" }: ACFishSeaViewProps) {
  const [mode, setMode] = useState<Mode>(initialMode);

  // If parent changes which tab is selected (fish vs sea), sync the toggle
  useEffect(() => {
    setMode(initialMode);
  }, [initialMode]);
  ...
  return (
    <View className="flex-1">
      ...
      {/* The actual grid */}
      {mode === "fish" ? <ACFishGrid search={search} /> : <ACSeaCreatureGrid search={search} />}
    </View>
  );
}
