// components/AnimalCrossing/ACItemsGrid.tsx
...
import { fetchItemByName, fetchItemNames, type NookipediaItem } from "@/lib/animalCrossing/nookipediaItems";
...
function buildItemImageCandidates(item?: NookipediaItem | null): string[] {
  if (!item) return [];
  return uniqStrings([(item as any)?.image_url, (item as any)?.render_url]);
}

type ACItemsGridProps = {
  search: string;
...
type ItemTileProps = {
  name: string;
  detail?: NookipediaItem;
  usedThumb?: number;
  isDetailLoading: boolean;
  onNeedFallbackThumb: (name: string) => Promise<void>;
};

const ItemTile: React.FC<ItemTileProps> = React.memo(({ name, detail, usedThumb, isDetailLoading, onNeedFallbackThumb }) => {
  const router = useRouter();

  const key = useMemo(() => acMakeKey("item", name), [name]);
  ...
  const candidates = useMemo(() => buildItemImageCandidates(detail), [detail]);
  ...
  const mat = detail?.material_type ? String((detail as any).material_type) : "Item";
    ...
    <View className="w-1/3 p-1">
      <View className="rounded-3xl p-3 border mb-1 bg-slate-900/80 border-slate-700 items-center">
        ...
        <View className="flex-row items-center mt-2">
          <Pressable
            onPress={() => {
              toggleCollected("item", name);
              if (!isCollected && count <= 0) setCount("item", name, 1);
            }}
...
const ACItemsGrid: React.FC<ACItemsGridProps> = ({ search, collectedOnly = false, collectedIds }) => {
  ...
  const collectedSet = useMemo(() => {
    ...
    for (const [key, entry] of Object.entries(entries || {})) {
      ...
      const name = key.slice("item:".length).trim();
  ...
  const grid = useACNameDetailGrid<NookipediaItem>({
    search,
    fetchNames: fetchItemNames,
    fetchDetail: fetchItemByName,
    pageSize: PAGE_SIZE,
  ...
  return (
    <ACGridWrapper<string>
      ...
      initialLoadingText={collectedOnly ? "Loading your collected items…" : "Loading items list…"}
      errorText={grid.namesError}
      onRetry={grid.retryReloadNames}
      isEmpty={grid.filteredNames.length === 0}
      emptyText={collectedOnly ? "No collected items in this category." : "No items match this search."}
