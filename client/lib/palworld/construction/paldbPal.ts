// lib/palworld/paldbPal.ts
//
// PalDB Construction -> Pal list + lightweight detail
// Source page: https://paldb.cc/en/Pal
//
// Mirrors your Construction scrapers (Storage, etc):
// - fetchPalList(): returns the build objects on /en/Pal
// - fetchPalDetail(slug): merges list-page parse + detail-page treant dependency tree
//
...
export type PalConstructionIndexItem = {
  slug: string; // e.g. "Palbox"
  name: string; // "Palbox"
  iconUrl: string | null; // size128 icon (or best available)

  categoryText: string | null; // typically "Pal" on this page

  technologyLevel: number | null; // "Technology 2"

  description: string | null;

  recipe: PalRecipeIngredient[];
};

export type PalDetail = PalConstructionIndexItem & {
...
function palDetailUrl(slug: string) {
...
function parseCardListPage(html: string): PalConstructionIndexItem[] {
  const src = String(html ?? "");
  if (!src) return [];

  const cards = extractDivBlocksByClass(src, "card itemPopup");

  const out: PalConstructionIndexItem[] = [];
  ...
  for (const card of cards) {
    ...
    const recipe: PalRecipeIngredient[] = [];
    if (recipesBlock) {
  ...
  const map = new Map<string, PalConstructionIndexItem>();
