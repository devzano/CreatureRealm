// lib/palworld/paldbAmmo.ts
//
// PalDB Ammo scraping helpers (index + detail) for CreatureRealm.
//
// Index: https://paldb.cc/en/Ammo
...
// (Optional) re-export kit types for consumers that import from paldbAmmo.ts
export type {
  DetailItemRef,
  DetailIngredient,
  DetailRecipeRow,
...
export type AmmoRecipeIngredient = {
  slug: string;
  name: string;
  iconUrl: string | null;
  qty: number;
...
export type AmmoIndexItem = {
  slug: string; // e.g. "Rocket_Ammo" OR "/en/Rocket_Ammo" depending on PalDB
  name: string; // e.g. "Rocket Ammo"
  iconUrl: string | null;

  rarity: string | null;
  technology: number | null; // “Technology” number

  description: string | null;
  isAvailable: boolean;

  // list-page snippet recipes
  recipes: AmmoRecipeIngredient[];
};

// -----------------------------
// Compat exports (UI parity with spheres)
// -----------------------------

export type AmmoListItem = AmmoIndexItem;

export async function fetchAmmoList(opts?: { force?: boolean }): Promise<AmmoIndexItem[]> {
  return fetchAmmoIndex(opts);
}

export async function warmAmmoList(): Promise<void> {
  return warmAmmoIndex();
}

// -----------------------------
// Types (Detail) - matches SphereDetail model
// -----------------------------
...
let _indexCache: AmmoIndexItem[] | null = null;
let _indexCacheAt = 0;
const _detailCache = new Map<string, { at: number; data: AmmoDetail }>();
...
export async function warmAmmoIndex(): Promise<void> {
  try {
    await fetchAmmoIndex({ force: false });
  } catch {
    // warm should never throw
  }
}

export async function fetchAmmoIndex(opts?: { force?: boolean }): Promise<AmmoIndexItem[]> {
  ...
  const url = "https://paldb.cc/en/Ammo";
  const html = await fetchHtml(url);

  const items = parseAmmoIndexHtml(html);
...
export async function fetchAmmoDetail(slugOrHref: string, opts?: { force?: boolean }): Promise<AmmoDetail> {
  const idRaw = String(slugOrHref ?? "").trim();
  if (!idRaw) throw new Error("fetchAmmoDetail: missing slug/href");

  const url = normalizeDetailHref(idRaw);
  if (!url) throw new Error("fetchAmmoDetail: unable to normalize href");
  ...
  const detail = parseAmmoDetailHtml(html, cacheKey);
...
// -----------------------------
// Index parsing (/en/Ammo) - matches Sphere parsing style
// -----------------------------

export function parseAmmoIndexHtml(html: string): AmmoIndexItem[] {
  const src = String(html ?? "");
  if (!src) return [];

  const chunks = src.split(/<div class="col">\s*<div class="card itemPopup">/i).slice(1);
  const items: AmmoIndexItem[] = [];
  ...
  const bySlug = new Map<string, AmmoIndexItem>();
...
function parseRecipes(recipesHtml: string): AmmoRecipeIngredient[] {
  ...
  const out: AmmoRecipeIngredient[] = [];
  ...
  const map = new Map<string, AmmoRecipeIngredient>();
  for (const r of out) {
...
function parseAmmoDetailHtml(html: string, slugKey: string): AmmoDetail {
  const src = String(html ?? "");

  const base: AmmoDetail = {
...
function mergeWithIndexFallback(detail: AmmoDetail): AmmoDetail {
  const index = _indexCache?.find((x) => cleanKey(x.slug) === cleanKey(detail.slug));
  if (!index) return detail;

  return {
    ...detail,
    // prefer detail’s values when present, else index
