// lib/palworld/paldbAccessory.ts
//
// PalDB Accessories (index + detail)
//
// Index: https://paldb.cc/en/Accessory
...
// (Optional) re-export kit types for consumers that import from paldbAccessory.ts
export type {
  DetailItemRef,
  DetailRecipeRow,
  TreantNode,
...
export type AccessoryRecipeIngredient = {
  slug: string;
  name: string;
  iconUrl: string | null;
  qty: number;
...
export type AccessoryIndexItem = {
  slug: string; // "Some_Accessory" (cleanKey)
  name: string;
  iconUrl: string | null;
  ...
  recipes: AccessoryRecipeIngredient[];
};
...
let _indexCache: AccessoryIndexItem[] | null = null;
let _indexCacheAt = 0;
const _detailCache = new Map<string, { at: number; data: AccessoryDetail }>();
...
export async function warmAccessoryIndex(): Promise<void> {
  try {
    await fetchAccessoryIndex({ force: false });
  } catch {
    // warm should never throw
  }
}

export async function fetchAccessoryIndex(opts?: { force?: boolean }): Promise<AccessoryIndexItem[]> {
  const force = !!opts?.force;
  const now = Date.now();

  if (!force && _indexCache && now - _indexCacheAt < INDEX_TTL) return _indexCache;

  const url = "https://paldb.cc/en/Accessory";
  const html = await fetchHtml(url);

  const items = parseAccessoryIndexHtml(html);
...
export async function fetchAccessoryDetail(slugOrHref: string, opts?: { force?: boolean }): Promise<AccessoryDetail> {
  const idRaw = String(slugOrHref ?? "").trim();
  if (!idRaw) throw new Error("fetchAccessoryDetail: missing slug/href");

  const url = normalizeDetailHref(idRaw);
  if (!url) throw new Error("fetchAccessoryDetail: unable to normalize href");
  ...
  const detail = parseAccessoryDetailHtml(html, cacheKey);
...
export type AccessoryListItem = AccessoryIndexItem;
export async function fetchAccessoryList(opts?: { force?: boolean }) {
  return fetchAccessoryIndex(opts);
}
export async function warmAccessoryList() {
  return warmAccessoryIndex();
}
...
// -----------------------------
// Index parsing (/en/Accessory)
// -----------------------------

export function parseAccessoryIndexHtml(html: string): AccessoryIndexItem[] {
  const src = String(html ?? "");
  if (!src) return [];

  const chunks = src.split(/<div class="col">\s*<div class="card itemPopup">/i).slice(1);
  const items: AccessoryIndexItem[] = [];
  ...
  const bySlug = new Map<string, AccessoryIndexItem>();
...
function parseRecipes(recipesHtml: string): AccessoryRecipeIngredient[] {
  ...
  const out: AccessoryRecipeIngredient[] = [];
  ...
  const map = new Map<string, AccessoryRecipeIngredient>();
  for (const r of out) {
...
// -----------------------------
// Detail parsing helpers: split desc vs effects (SphereModule parity)
// -----------------------------
...
function parseAccessoryDetailHtml(html: string, slugKey: string): AccessoryDetail {
  const src = String(html ?? "");

  const base: AccessoryDetail = {
...
function mergeWithIndexFallback(detail: AccessoryDetail): AccessoryDetail {
