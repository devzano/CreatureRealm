// lib/palworld/paldbWeapon.ts
//
// PalDB Weapon scraping helpers (index + detail) for CreatureRealm.
//
// Index: https://paldb.cc/en/Weapon
// Detail: https://paldb.cc/en/<slug>
// Uses shared detail section parsing model via paldbDetailKit
//
// Output sections (same parity as Material):
// - stats, others
// - producedAt
...
export type WeaponRecipeIngredient = {
  slug: string;
  name: string;
  iconUrl: string | null;
  qty: number;
...
// Compat exports
export type WeaponListItem = WeaponIndexItem;

export async function fetchWeaponList(opts?: { force?: boolean }): Promise<WeaponIndexItem[]> {
  return fetchWeaponIndex(opts);
}

export async function warmWeaponList(): Promise<void> {
  return warmWeaponIndex();
}
...
let _indexCache: WeaponIndexItem[] | null = null;
let _indexCacheAt = 0;
const _detailCache = new Map<string, { at: number; data: WeaponDetail }>();
...
export async function warmWeaponIndex(): Promise<void> {
  try {
    await fetchWeaponIndex({ force: false });
  } catch {
    // warm should never throw
  }
}

export async function fetchWeaponIndex(opts?: { force?: boolean }): Promise<WeaponIndexItem[]> {
  ...
  const url = "https://paldb.cc/en/Weapon";
  const html = await fetchHtml(url);

  const items = parseWeaponIndexHtml(html);
...
export async function fetchWeaponDetail(slugOrHref: string, opts?: { force?: boolean }): Promise<WeaponDetail> {
  const idRaw = String(slugOrHref ?? "").trim();
  if (!idRaw) throw new Error("fetchWeaponDetail: missing slug/href");

  const url = normalizeDetailHref(idRaw);
  if (!url) throw new Error("fetchWeaponDetail: unable to normalize href");

  // NOTE: detail endpoint is by slug/href, so the cache is by slug/href.
  // Variant handling (same slug, different rarity) should be done by the UI cache key.
  ...
  const detail = parseWeaponDetailHtml(html, cacheKey);
...
// IMPORTANT:
// PalDB often lists multiple variants (same slug/name) with different rarity.
// Do NOT dedupe by slug alone, or you'll collapse the variants.
function indexVariantKey(slug: string, rarity: string | null) {
  return `${cleanKey(slug)}::${cleanKey(rarity ?? "")}`;
}

export function parseWeaponIndexHtml(html: string): WeaponIndexItem[] {
  const src = String(html ?? "");
  if (!src) return [];

  const chunks = src.split(/<div class="col">\s*<div class="card itemPopup">/i).slice(1);
  const items: WeaponIndexItem[] = [];
  ...
  // Dedupe by (slug + rarity) instead of slug.
  const byVariant = new Map<string, WeaponIndexItem>();
  for (const it of items) {
    const k = indexVariantKey(it.slug, it.rarity);
    if (!byVariant.has(k)) byVariant.set(k, it);
  }
  return Array.from(byVariant.values());
}

function parseRecipes(recipesHtml: string): WeaponRecipeIngredient[] {
  ...
  const out: WeaponRecipeIngredient[] = [];
  ...
  const map = new Map<string, WeaponRecipeIngredient>();
...
function mergeWithIndexFallback(detail: WeaponDetail): WeaponDetail {
  // Prefer an index row that matches both slug + rarity when possible,
  // but fall back to first slug match if the detail page doesn't disambiguate.
  const slugKey = cleanKey(detail.slug);
  const rarityKey = cleanKey(detail.rarity ?? "");

  const exact =
    _indexCache?.find((x) => cleanKey(x.slug) === slugKey && cleanKey(x.rarity ?? "") === rarityKey) ?? null;

  const loose = _indexCache?.find((x) => cleanKey(x.slug) === slugKey) ?? null;

  const index = exact ?? loose;
