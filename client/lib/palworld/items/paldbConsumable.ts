// lib/palworld/paldbConsumable.ts
//
// PalDB Consumable scraping helpers (index + detail) for CreatureRealm.
//
// Index: https://paldb.cc/en/Consumable
...
export type ConsumableRecipeIngredient = {
  slug: string;
  name: string;
  iconUrl: string | null;
  qty: number;
...
export type ConsumableIndexItem = {
  slug: string;
  name: string;
  iconUrl: string | null;
  ...
  recipes: ConsumableRecipeIngredient[];
};

export type ConsumableListItem = ConsumableIndexItem;

export async function fetchConsumableList(opts?: { force?: boolean }): Promise<ConsumableIndexItem[]> {
  return fetchConsumableIndex(opts);
}

export async function warmConsumableList(): Promise<void> {
  return warmConsumableIndex();
}
...
let _indexCache: ConsumableIndexItem[] | null = null;
let _indexCacheAt = 0;
const _detailCache = new Map<string, { at: number; data: ConsumableDetail }>();

const INDEX_TTL = 10 * 60 * 1000;
const DETAIL_TTL = 10 * 60 * 1000;

export async function warmConsumableIndex(): Promise<void> {
  try {
    await fetchConsumableIndex({ force: false });
  } catch {
    // warm should never throw
  }
}

export async function fetchConsumableIndex(opts?: { force?: boolean }): Promise<ConsumableIndexItem[]> {
  ...
  const url = "https://paldb.cc/en/Consumable";
  const html = await fetchHtml(url);

  const items = parseConsumableIndexHtml(html);
...
export async function fetchConsumableDetail(
  slugOrHref: string,
  opts?: { force?: boolean }
): Promise<ConsumableDetail> {
  const idRaw = String(slugOrHref ?? "").trim();
  if (!idRaw) throw new Error("fetchConsumableDetail: missing slug/href");

  const url = normalizeDetailHref(idRaw);
  if (!url) throw new Error("fetchConsumableDetail: unable to normalize href");
  ...
  const detail = parseConsumableDetailHtml(html, cacheKey);
...
export function parseConsumableIndexHtml(html: string): ConsumableIndexItem[] {
  const src = String(html ?? "");
  if (!src) return [];

  const chunks = src.split(/<div class="col">\s*<div class="card itemPopup">/i).slice(1);
  const items: ConsumableIndexItem[] = [];
  ...
  // Some consumables (e.g., Treasure Maps) can share the same slug/name but differ by rarity.
  // Dedupe using a composite key so variants aren't dropped.
  const byKey = new Map<string, ConsumableIndexItem>();
...
function parseRecipes(recipesHtml: string): ConsumableRecipeIngredient[] {
  ...
  const out: ConsumableRecipeIngredient[] = [];
  ...
  const map = new Map<string, ConsumableRecipeIngredient>();
  for (const r of out) if (!map.has(r.slug)) map.set(r.slug, r);
  return Array.from(map.values());
}

function parseConsumableDetailHtml(html: string, slugKey: string): ConsumableDetail {
  const src = String(html ?? "");

  const base: ConsumableDetail = {
...
function mergeWithIndexFallback(detail: ConsumableDetail): ConsumableDetail {
